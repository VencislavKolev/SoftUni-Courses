namespace P01_InitialSetup
{
    ...
    class P01_InitialSetup
    {
        ...
        {
            using SqlConnection dbCon = new SqlConnection(@"Server=.;Integrated Security=true");
            dbCon.Open();

            SqlCommand createDB = new SqlCommand("CREATE DATABASE MinionsDB", dbCon);
            createDB.ExecuteNonQuery();

            using SqlConnection connection = new SqlConnection(@"Server=.;Database=MinionsDB;Integrated Security=true");
            connection.Open();

            string[] tables = ("CREATE TABLE Countries (Id INT PRIMARY KEY IDENTITY,Name VARCHAR(50))" + Environment.NewLine +
            ...
            string[] inserts = ("INSERT INTO Countries ([Name]) VALUES ('Bulgaria'),('England'),('Cyprus'),('Germany'),('Norway')" + Environment.NewLine +
